...
public class JainSipCallListener implements ICallListener {
    ...
    public void onCallStarting(ICallStartingEvent event) {
        ...
        IMessage message = new AlenaMessage(MessageType.CALL_STARTING);
        ...
        if(null != ctx){
            ...
            message.set(AlenaMessage.PROP_STAGE, event.getStage().toString());
            message.set(AlenaMessage.PROP_SDP, ((ICallParams)event.getCall()).getSdpObject().getOfferer());
            ctx.onEvent(new CallEvent((AlenaMessage)message));
        } else {
    ...
    public void onCallStarted(ICallEvent event) {
        log.info(event.toString());
        IMessage message = new AlenaMessage(MessageType.CALL_STARTED);
        ICallContext ctx = app.findCallContext(event.getCall().getId());
        if(null != ctx){
            message.set(AlenaMessage.PROP_SESSION_ID, ctx.getSessionId());
            message.set(AlenaMessage.PROP_CALL_ID, ctx.getCallId());
            message.set(AlenaMessage.PROP_SDP, ((ICallParams)event.getCall()).getSdpObject().getAnswerer());
            ctx.onEvent(new CallEvent((AlenaMessage)message));
        } else {
            log.warn("Call " + event.getCall().getId() + " not found");
        }
    ...
    public void onCallFinished(ICallEvent event) {
        log.info(event.toString());
        IMessage message = new AlenaMessage(MessageType.CALL_FINISHED);
        ICallContext ctx = app.findCallContext(event.getCall().getId());
        if(null != ctx){
            message.set(AlenaMessage.PROP_SESSION_ID, ctx.getSessionId());
            message.set(AlenaMessage.PROP_CALL_ID, ctx.getCallId());
    ...
    public void onCallFailed(ICallFailedEvent event) {
        log.info(event.toString());
        IMessage message = new AlenaMessage(MessageType.CALL_FAILED);
        ICallContext ctx = app.findCallContext(event.getCall().getId());
        if(null != ctx){
            message.set(AlenaMessage.PROP_SESSION_ID, ctx.getSessionId());
            message.set(AlenaMessage.PROP_CALL_ID, ctx.getCallId());
            message.set(AlenaMessage.PROP_REASON, event.getRejectReason().toString());
            ctx.onEvent(new CallEvent((AlenaMessage)message));
        } else {
            log.warn("Call " + event.getCall().getId() + " not found");
        }
    ...
    public void onCallError(ICallEvent event) {
        ...
        if(null != ctx){
            ...
            if(null == e){
                message.set(AlenaMessage.PROP_REASON, "ERROR");
            }else{
                message.set(AlenaMessage.PROP_REASON, e.getErrorCode() + " : " + e.getErrorText());
            }
